...
从特定限速器申请令牌, 如果限速器没有可用令牌, 则阻塞直到有新令牌.
id用于标识限速器.

该项目中的令牌是一种类似锁的抽象概念, 与令牌桶(token bucket)无关.
内部使用的算法类似于漏桶(leaky bucket), 但桶中的水量是无限的.

如果开启基于token的访问控制, 则可能需要在Querystring提供具有read权限的token:
=GET /geyser/<id>?token=<token>=

当一个限速器未配置duration和limit时, acquire操作是无法进行的, 此时将会返回503.
...
可用以下环境变量作为全局设置:
=GEYSER_DURATION=, 没有默认值.
=GEYSER_LIMIT=, 没有默认值.

***** 获取所有具有配置的geyser id
=GET /admin/geyser-with-configurations=
...
curl \
  ...
  "http://localhost:8080/admin/geyser-with-configurations"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch('http://localhost:8080/admin/geyser-with-configurations', {
  headers: {
...
***** 获取特定限速器的配置
=GET /admin/geyser/<id>/configurations=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/geyser/$id/configurations"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/geyser/${id}/configurations`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
***** 设置配置
=PUT /admin/geyser/<geyser-id>/configurations/duration=
=PUT /admin/geyser/<geyser-id>/configurations/limit=
...
curl \
  ...
  --data "$DURATION" \
  "http://localhost:8080/admin/geyser/$id/configurations/duration"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/geyser/${id}/configurations/duration`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
...
***** 移除配置
=DELETE /admin/geyser/<geyser-id>/configurations/duration=
=DELETE /admin/geyser/<geyser-id>/configurations/limit=
...
- =whitelist=
  启用基于限制器白名单的访问控制, 只有在名单内的限制器允许被访问.
- =blacklist=
  启用基于限制器黑名单的访问控制, 只有在名单外的限制器允许被访问.

****** 黑名单
******* 获取黑名单
=GET /admin/blacklist=

获取位于黑名单中的所有限制器id, 返回JSON表示的字符串数组 =string[]=.
...
基于token的访问控制将根据限制器的token access policy决定其访问规则.
可通过环境变量 =GEYSER_ACQUIRE_TOKEN_REQUIRED= 设置相关默认值,
未设置情况下为 =false=.

一个限制器可以有多个token, 每个token可以单独设置acquire和read权限, 不同限制器的token不共用.
...
获取所有具有token策略的限制器id, 返回由JSON表示的字符串数组 =string[]=.
...
获取所有具有token的限制器id, 返回由JSON表示的字符串数组 =string[]=.
...
获取特定限制器的所有token, 返回JSON数组表示的token及其权限
